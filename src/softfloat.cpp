...
// Modifications made for uwin jit:
//  > C++-ise, put to a namespace
...
#define packFloatx80Constant(sign, exponent, mantissa) { .fraction = mantissa, .exp = (sign << 15) | exponent }
#define packFloat128Constant(high, low) { .lo = low, .hi = high }
...
#include "libx87/softfloat/softfloat-macros.h"
...
| specific.
*----------------------------------------------------------------------------*/
#include "libx87/softfloat/softfloat-specialize.h"

#include "libx87/softfloat/fpu-constants.h"
...
floatx80 SoftFloatRoundAndPackFloatx80(int roundingPrecision,
                                       int zSign, int32_t zExp, uint64_t zSig0, uint64_t zSig1,
...
/* executes single exponent reduction cycle */
    ...
    static int
    do_fprem(floatx80 a, floatx80 b, floatx80 *r_input, uint64_t *q_input, int rounding_mode, float_status_t *status) {
        floatx80 r = *r_input;
        uint64_t q = *q_input;
